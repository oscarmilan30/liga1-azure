...
## 15b. GitHub Actions — Los 3 Workflows

El repositorio tiene tres workflows en `.github/workflows/` que automatizan el ciclo completo de la plataforma. Ninguno contiene credenciales — todas las claves se leen desde los secrets de GitHub.

### `liga1-deploy-prod.yml` — Despliegue a producción

Se dispara automáticamente cuando hay un push a `main` que afecta `proceso/`, `PrepAmb/`, `datasets/dataentrys/` o el propio archivo del workflow. También se puede ejecutar manualmente (workflow_dispatch).

Ejecuta 5 jobs en orden:

1. **Deploy ADF Prod**: Despliega los Linked Services, Datasets y Pipelines de Azure Data Factory en el entorno de producción, reemplazando las referencias de dev por las de prod. Configura los Global Parameters (host Databricks, usuario, cluster ID).
2. **Deploy Azure SQL Prod**: Ejecuta `PrepAmb/Querys.sql` en la base de datos `ligafutbol-prod` para crear las tablas de control del orquestador (solo si no existen — es idempotente).
3. **Deploy Databricks Prod**: Crea el catalog `catalog_liga1_prod`, schemas y Volume en Unity Catalog. Ejecuta todos los DDL de `PrepAmb/ddl_deploy/` reemplazando variables. Configura el secrets scope con credenciales prod. Sube el wheel `liga1_utils` al Volume. Configura Delta Sharing e imprime el Activation Link para Power BI.
4. **Actualizar Job IDs**: Lee los IDs reales asignados por Databricks a los jobs y los actualiza en `tbl_pipeline_parametros` de SQL prod, para que el orquestador ADF los use correctamente.
5. **Upload Dataentrys → ADLS prod**: Sube los archivos estáticos de `datasets/dataentrys/` (catálogos, plantillas manuales, posiciones, etc.) a `primera_division/landing/dataentrys/` en ADLS prod. Solo corre cuando hay cambios en esos archivos.

---

### `liga1-scraping.yml` — Scraping y carga de datos

Se ejecuta manualmente (workflow_dispatch) con los parámetros `modo`, `ambiente` y opcionalmente `anio_inicio`/`anio_fin`. Soporta 4 modos:

- **incremental**: Scrapea FotMob, Transfermarkt y Wikipedia para la temporada actual y sube los archivos al ADLS del ambiente seleccionado.
- **reproceso**: Igual que incremental pero para un año específico (`anio_objetivo`).
- **historica**: Lanza un matrix de jobs en paralelo (máximo 2 simultáneos), uno por año desde `anio_inicio` hasta `anio_fin`, cada uno corriendo en modo reproceso. Al terminar todos los años, el job `trigger-adf` ejecuta un fallback que sube desde `datasets/{año}/` del repositorio los archivos que no llegaron a ADLS por errores de scraping.
- **repair_tm**: Reprocesa solo los datos de Transfermarkt para un año específico, sin tocar FotMob ni Wikipedia.

Al terminar cualquier modo de scraping exitoso, el job `trigger-adf` actualiza los parámetros SQL (fecha, año) y dispara el pipeline `pl_Orchestrator_E2E_liga1` en ADF para procesar los datos recién llegados al landing.

---

### `liga1-trigger-adf.yml` — Disparo manual de ADF

Workflow de uso operativo para disparar el pipeline E2E de ADF sin necesidad de correr scraping. Útil cuando los datos ya están en ADLS landing y solo se quiere reprocesar las capas RDV → UDV → DDV → ML.

Acepta 3 parámetros: `modo` (INCREMENTAL, HISTORICO, REPROCESO), `anio_objetivo` (solo para REPROCESO) y `ambiente` (dev o prod). Actualiza los parámetros en SQL y dispara el orchestrator en el ADF del ambiente seleccionado.

---

## 16. Configurar Azure OpenAI — Asistente RAG

Azure OpenAI provee el modelo de lenguaje que recibe los datos de la liga como contexto y genera respuestas en lenguaje natural.

### 16.1 Crear el recurso Azure OpenAI

1. Portal Azure → barra de búsqueda → **Azure OpenAI**
2. Clic en **Crear**
3. Completar:
   - **Suscripción**: seleccionar tu suscripción
   - **Grupo de recursos**: `rg-liga1`
   - **Región**: **East US**
   - **Nombre**: `aoai-liga1`
   - **Plan de tarifa**: `Standard S0`
4. Clic en **Revisar y crear** → **Crear**
5. Una vez creado, ir al recurso → **Keys and Endpoint**
6. Copiar **Key 1** (se subirá a Key Vault en el paso siguiente — no guardar en el repo)
7. Copiar el **Endpoint** (formato: `https://aoai-liga1.openai.azure.com/`)

### 16.2 Guardar la API Key en Key Vault

1. Portal → `kv-liga1-secreto` → **Secretos** → **Generar o importar**
2. Nombre: `aoai-api-key`
3. Valor: la Key 1 copiada en el paso anterior
4. Clic en **Crear**

### 16.3 Crear el deployment del modelo

1. Ir al recurso `aoai-liga1` → **Azure OpenAI Studio** (botón en la página del recurso)
2. Menú izquierdo → **Deployments** → **+ Create new deployment**
3. Completar:
   - **Model**: `gpt-4o-mini` (o `gpt-5.4-mini` si aparece disponible en tu región)
   - **Deployment name**: `gpt-5.4-mini`
   - **Deployment type**: **Data Zone Standard** (o Standard si no está disponible)
   - **Version**: la más reciente disponible
4. Clic en **Deploy** → esperar estado **Succeeded**

> **Costo:** $0.75/1M tokens de entrada + $4.50/1M tokens de salida. Una consulta RAG típica usa ~4K tokens → ~$0.003 por pregunta.

### 16.4 Verificar

En **Azure OpenAI Studio** → **Chat Playground** → seleccionar `gpt-5.4-mini` → enviar cualquier mensaje de prueba.

---

## 17. Desplegar Databricks App — Asistente RAG Liga 1

La Databricks App es un servidor Streamlit dentro del workspace. Los valores sensibles se inyectan como Databricks Secrets — nunca viajan al repositorio.

### 17.1 Crear el PAT para la app

1. Workspace `dbw-liga1` → esquina superior derecha → tu nombre de usuario → **Settings** → **Developer** → **Access tokens** → **Generate new token**
2. Descripción: `rag-app-liga1`
3. Lifetime: `365` días
4. **Scope del token:** solo necesita **SQL** (leer el warehouse, no necesita crear clusters ni jobs)
5. Clic en **Generate** → copiar el token `dapi...` (**se muestra una sola vez**)

> Si ya tienes el token `powerbi-liga1` con scope SQL, **no lo reutilices** — crea uno dedicado para poder rotarlos de forma independiente.

### 17.2 Guardar el PAT en Key Vault

1. Portal → `kv-liga1-secreto` → **Secretos** → **Generar o importar**
2. Nombre: `rag-databricks-token`
3. Valor: el token `dapi...`
4. Clic en **Crear**

### 17.3 Verificar secrets en el scope

Desde un notebook de Databricks, ejecutar:

```python
dbutils.secrets.list("secretliga1")
```

Deben aparecer `aoai-api-key` y `rag-databricks-token` en la lista (además de los secretos anteriores). Si no aparecen de inmediato, esperar 2–3 minutos — el scope respaldado por Key Vault tiene un caché de propagación.

### 17.4 Crear la app en Databricks

1. Workspace `dbw-liga1` → menú lateral → **Apps**
2. Clic en **Create app**
3. Seleccionar **Custom** (no una plantilla)
4. Completar:
   - **App name**: `liga1-ia`
   - **Description**: `Asistente RAG Liga 1 Perú`
5. En **Source code**: seleccionar el repo `liga1-azure` → carpeta `databricks_app/`
6. Clic en **Next**

### 17.5 Configurar variables de entorno

Las variables no-sensibles ya están en `app.yaml` (se despliegan con el código). **No es necesario ingresarlas manualmente.** El `app.yaml` contiene:

| Variable | Valor |
|----------|-------|
| `AOAI_ENDPOINT` | `https://aoai-liga1.openai.azure.com/` |
| `AOAI_DEPLOYMENT` | `gpt-5.4-mini` |
| `AOAI_API_VERSION` | `2025-01-01-preview` |

`DATABRICKS_HOST` lo inyecta Databricks automáticamente. `DATABRICKS_HTTP_PATH` y `LIGA1_CATALOG` se leen desde el scope `secretliga1` — el mismo código funciona en dev y prod sin tocar `app.yaml`.

> **¿Por qué no hay `AOAI_API_KEY` ni `DATABRICKS_TOKEN` aquí?** El mecanismo de App Resources (Settings → Resources) **no funciona cuando el scope está respaldado por Key Vault** — los secrets no se inyectan como env vars. En lugar de eso, `rag.py` usa el SDK de Databricks para leerlos directamente en runtime:
>
> ```python
> from databricks.sdk import WorkspaceClient
> w = WorkspaceClient()   # usa CLIENT_ID + CLIENT_SECRET inyectados automáticamente
> secret = w.secrets.get_secret("secretliga1", "aoai-api-key")
> ```
>
> Databricks Apps siempre inyecta `DATABRICKS_CLIENT_ID` y `DATABRICKS_CLIENT_SECRET` — estas permiten al SDK llamar a la Secrets API sin configuración adicional.

### 17.6 Desplegar

1. Clic en **Deploy** (o **Create and deploy**)
2. Databricks instala `requirements.txt` y arranca Streamlit con el comando de `app.yaml`
3. El proceso tarda ~2–3 minutos
4. Cuando el estado cambie a **Running** → aparece la URL de la app

### 17.7 Probar la app

Abrir la URL y enviar estas preguntas de prueba:

- **"¿Cuál es la tabla de posiciones actual?"** → debe devolver datos de `ft_rendimiento_posiciones_vw`
- **"¿Cuál es el plantel de Alianza Lima en 2026?"** → debe devolver jugadores de `ft_plantillas_historico_vw`
- **"¿Quiénes son los goleadores del Clausura 2025?"** → debe filtrar por `tipo_tabla LIKE '%Clausura%'`

### 17.8 Gestión de costos

La app cobra solo cuando está en estado **Running** (~$0.07–0.10/DBU·h). Al terminar:
- **Apps** → `liga1-ia` → **Stop**
- Para reactivar: mismo menú → **Start** (~30 segundos)
- El warehouse `b74f62ff6067bdf7` ya está activo para Power BI → $0 incremental

---

## 18. Despliegue a Producción — CI/CD Automatizado
...
> **App RAG en prod:** El deploy de la app `liga1-ia` está **automatizado en el Job 6** del workflow `liga1-deploy-prod.yml`. Al hacer push a `main` con cambios en `databricks_app/`, el workflow la crea (si no existe) y la deploya automáticamente. Los secrets de entorno (`catalogname`, `databricks-http-path`, `aoai-api-key`, `rag-databricks-token`) los escribe el Job 3 en el scope `secretliga1` del workspace prod — sin pasos manuales.
...
```bash
git checkout develop && git merge test-construccion && git push origin develop
...
- **Build y sube wheel `liga1_utils`**: en cada deploy compila `proceso/util/utils_liga1.py` → `.whl` y lo sube a `/Volumes/catalog_liga1_prod/shared/libs/`. **Cualquier cambio en `utils_liga1.py` se despliega automáticamente con el siguiente push a `main` — no se requiere acción manual.**
- Los 5 jobs Databricks tienen el wheel configurado como **librería por task** (referenciando la ruta del Volume). Esto significa que cada ejecución del job carga el wheel directamente del Volume, siempre con la versión más reciente, sin depender del estado del cluster ni de reinicios.
- El deploy también intenta instalar el wheel a nivel de cluster vía API. Si el token no tiene el scope `libraries` (error 403), avisa pero no falla — no es necesario para los jobs. La instalación manual en el cluster (`Compute → Libraries`) es opcional y solo aplica para notebooks interactivos.
...
| Wheel | Compila desde `utils_liga1.py` y sobreescribe el `.whl` en el Volume. Los jobs lo cargan frescos vía job-task library. | Siempre la versión más nueva, sin reiniciar cluster |
...
### 16.8 Despliegue del Asistente IA (Databricks App)

El asistente RAG se despliega **automáticamente** como parte del mismo push a `main`. El workflow `liga1-deploy-prod.yml` incluye un **Job 6 — Deploy Databricks App** que:

1. Espera a que el Job 3 sincronice el repo en el workspace prod
2. Crea la app `liga1-ia` si no existe
3. Deploya desde `/Repos/{user}/liga1-azure/databricks_app`

**Prerequisito antes del primer deploy:** agregar el GitHub Secret `AOAI_API_KEY` con la key del recurso `aoai-liga1` (Azure Portal → `aoai-liga1` → Keys and Endpoint → KEY 1).

Los secrets de la app (`aoai-api-key`, `rag-databricks-token`, `catalogname`, `databricks-http-path`) los escribe automáticamente el Job 3 en el scope `secretliga1` del workspace prod. No se requiere configuración manual en Databricks UI.

Ver documentación completa del módulo IA en [05_ia_rag.md](05_ia_rag.md).

### 16.9 Verificar y capturar evidencias

Guardar capturas en `evidencias/`:

- GitHub Actions: los 6 jobs en ✅, URL de la app visible en logs del job 6
...
- Databricks prod: app `liga1-ia` en estado **Running** con URL visible

---

*Guía de Despliegue — Liga 1 Perú Data Engineering Platform · Oscar García Del Águila · 2025–2026 · Secciones 16–17: Módulo IA RAG*
