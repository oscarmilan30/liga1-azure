...
CREATE OR REPLACE TABLE ${catalog_name}.tb_udv.md_plantillas (
  id_jugador                   STRING NOT NULL COMMENT 'Identificador único del jugador: sha2(id_tm, 256) — estable e inmune a cambios de nombre/fecha/nacionalidad',
  id_tm                        STRING COMMENT 'ID del jugador en Transfermarkt (fuente externa)',
...
USING delta
LOCATION 'abfss://${container_name}@${storage_account}.dfs.core.windows.net/${ruta_base}/udv/md_plantillas/data'
...
CREATE OR REPLACE TABLE ${catalog_name}.tb_udv.hm_plantillas_equipo (
  ...
  id_tm                       STRING COMMENT 'ID del jugador en Transfermarkt (facilita JOIN con estadisticas_jugadores)',
...
LOCATION 'abfss://${container_name}@${storage_account}.dfs.core.windows.net/${ruta_base}/udv/hm_plantillas_equipo/data'
