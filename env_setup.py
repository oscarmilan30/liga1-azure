# ==========================================================
# ENV_SETUP.PY - VERSIÓN CORREGIDA
# ==========================================================
...
# ----------------------------------------------------------
# SINCRONIZACIÓN CON GITHUB - VERSIÓN SIMPLIFICADA
# ----------------------------------------------------------
def sync_with_github():
    ...
    try:
        ...
        # Usar directorio en /Workspace que tiene permisos
        github_dir = "/Workspace/liga1_github_main"
        ...
        if not os.path.exists(github_dir):
            ...
            if reset_result.returncode != 0:
                print(f"[ENV_SETUP ERROR] Reset falló: {reset_result.stderr}")
                return None
        ...
        print(f"[ENV_SETUP]   Código actualizado - Commit: {current_commit}")
        return github_dir
...
# ----------------------------------------------------------
try:
    # Detectar raíz del repo original (snapshot)
    original_root = detect_repo_root()
    print(f"[ENV_SETUP] Snapshot del Job: {original_root}")
    ...
    if github_root:
        ...
        print(f"[ENV_SETUP]  Usando snapshot del Job (falló GitHub)")

    # Configurar importaciones
    if repo_root not in sys.path:
        sys.path.append(repo_root)

    auto_import_modules(repo_root, verbose=True)
    spark = get_or_create_spark(verbose=True)

    print(f"[ENV_SETUP] INICIALIZACIÓN COMPLETADA")
