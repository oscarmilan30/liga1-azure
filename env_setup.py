...
# Versión: 3.6 (SmartBranch + CleanLogs + Dynamic Imports)
# ==========================================================
# Compatible con:
...
#
# Características:
#   Detección automática del modo (interactive / job / local)
#   Refresco Git (solo en Jobs Git)
#   Importación dinámica de subcarpetas (sin hardcode)
#   Construcción de rutas absolutas para YAML u otros archivos
#   Detección de branch y commit (sin errores si es snapshot)
#   Verbosidad adaptativa (develop = debug, main = silencioso)
#   Limpieza de logs y supresión de “fatal: not a git repository”
# ==========================================================
...
def detect_repo_root(verbose=False) -> str:
    try:
        ...
        if "/Repos/.internal" in cwd:
            ...
            # Detectar subcarpeta del proyecto
...
def refresh_repo_if_needed(repo_root: str, verbose=False):
    try:
        ...
        subprocess.run(["git", "-C", repo_root, "fetch", "--all"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["git", "-C", repo_root, "reset", "--hard", "origin/main"], check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if verbose:
            print("[ENV_SETUP] Repo sincronizado con origin/main")
    except Exception:
        if verbose:
            print("[ENV_SETUP] Advertencia: no se pudo refrescar el repo Git (modo snapshot).")

# ----------------------------------------------------------
# OBTENER BRANCH ACTIVO Y COMMIT (seguro)
# ----------------------------------------------------------
def get_git_info(repo_root: str):
    ...
    try:
        ...
        elif os.path.exists(os.path.join(repo_root, ".git")):
