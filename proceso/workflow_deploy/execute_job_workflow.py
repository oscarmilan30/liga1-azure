...
dbutils.widgets.text("prm_user", "")
dbutils.widgets.text("prm_cluster_id", "")

prm_job_name = dbutils.widgets.get("prm_job_name")
prm_area = dbutils.widgets.get("prm_area")
prm_user = dbutils.widgets.get("prm_user")
prm_cluster_id = dbutils.widgets.get("prm_cluster_id")
...
def cargar_definicion_job(nombre_job):
    ...
    with open(ruta_json, 'r', encoding='utf-8') as archivo:
        ...
        # Reemplazar placeholders genéricos con valores del parámetro SQL
        job_str = json.dumps(job_definition)
        if prm_user:
            job_str = job_str.replace("{USER}", prm_user)
        if prm_cluster_id:
            job_str = job_str.replace("{CLUSTER_ID}", prm_cluster_id)
        job_definition = json.loads(job_str)
