...
from utils_liga1 import get_dbutils, get_workspace_path
...
# Resolver catalog_name: secrets scope > widget > fallback
# En prod el scope tiene catalogname=catalog_liga1_prod; en dev no existe → fallback
try:
    catalog_name = dbutils.secrets.get(scope="secretliga1", key="catalogname")
    print(f"[INFO] catalog_name leído desde secrets scope: {catalog_name}")
except Exception:
    dbutils.widgets.text("catalog_name", "catalog_liga1")
    catalog_name = dbutils.widgets.get("catalog_name")
    print(f"[INFO] catalog_name desde widget/fallback: {catalog_name}")
...
def execute_ddl_file(file_path):
    ...
    try:
        ...
        ddl_content = ddl_content.replace("${ruta_base}", "primera_division")
