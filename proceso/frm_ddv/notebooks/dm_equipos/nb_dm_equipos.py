...
from dm_equipos import carga_final_dm_equipos
...
log("Inicio DDV dm_equipos", "INFO", "dm_equipos")
...
# ----------------------------------------------------------
try:
    dbutils.widgets.text("prm_pipelineid",   "27")
    ...
    prm_ruta_md_catalogo   = dict_predecesores["md_catalogo_equipos"]["Ruta_Predecesor"]
    prm_ruta_md_equipos    = dict_predecesores["md_equipos"]["Ruta_Predecesor"]
    prm_ruta_md_estadios   = dict_predecesores["md_estadios"]["Ruta_Predecesor"]
    ...
    log(f"Error parámetros: {e}", "ERROR", "dm_equipos")
...
try:
    ...
    prm_cols_md_catalogo   = cfg["cols_md_catalogo"]
    prm_cols_md_equipos    = cfg["cols_md_equipos"]
    prm_cols_md_estadios   = cfg["cols_md_estadios"]
...
# ----------------------------------------------------------
try:
    df_catalogo   = read_delta_adls(spark, prm_ruta_md_catalogo)
    df_equipos    = read_delta_adls(spark, prm_ruta_md_equipos)
    df_estadios   = read_delta_adls(spark, prm_ruta_md_estadios)
    df_valoracion = read_delta_adls(spark, prm_ruta_hm_valoracion)

    if is_dataframe_empty(df_catalogo):
        raise Exception(f"md_catalogo_equipos vacío: {prm_ruta_md_catalogo}")

    log("Tablas UDV leídas", "INFO", entity_name)
except Exception as e:
    log(f"Error lectura UDV: {e}", "ERROR", entity_name)
    print(traceback.format_exc())
    raise
...
# ----------------------------------------------------------
cnt_entrada = df_catalogo.count()
cnt_salida  = 0
try:
    df_final = carga_final_dm_equipos(
        df_catalogo,   prm_cols_md_catalogo,
        df_equipos,    prm_cols_md_equipos,
        df_estadios,   prm_cols_md_estadios,
    ...
    log("dm_equipos cargado correctamente", "SUCCESS", entity_name)
except Exception as e:
...
log("Fin DDV dm_equipos", "INFO", entity_name)
