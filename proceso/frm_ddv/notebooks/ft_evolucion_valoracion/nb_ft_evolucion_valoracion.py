...
from ft_evolucion_valoracion import carga_final_ft_evolucion_valoracion
...
log("Inicio DDV ft_evolucion_valoracion", "INFO", "ft_evolucion_valoracion")
...
# ----------------------------------------------------------
try:
    dbutils.widgets.text("prm_pipelineid",   "32")
    ...
    log(f"Error parámetros: {e}", "ERROR", "ft_evolucion_valoracion")
...
# ----------------------------------------------------------
try:
    df_hm_valoracion = read_delta_adls(spark, prm_ruta_hm_valoracion)
    df_dm_equipos    = read_delta_adls(spark, prm_ruta_dm_equipos)

    if prm_periodo > 0:
        df_hm_valoracion = df_hm_valoracion.filter(col("periodo") == prm_periodo)

    if is_dataframe_empty(df_hm_valoracion):
        raise Exception(f"hm_valoracion_equipos vacía: {prm_ruta_hm_valoracion}")
...
# ----------------------------------------------------------
cnt_entrada = df_hm_valoracion.count()
cnt_salida  = 0
try:
    df_final = carga_final_ft_evolucion_valoracion(
        df_hm_valoracion, prm_cols_hm_valoracion,
    ...
    log("ft_evolucion_valoracion cargado correctamente", "SUCCESS", entity_name)
except Exception as e:
...
log("Fin DDV ft_evolucion_valoracion", "INFO", entity_name)
