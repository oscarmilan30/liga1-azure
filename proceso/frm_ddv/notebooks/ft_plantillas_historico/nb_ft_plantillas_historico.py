...
from ft_plantillas_historico import carga_final_ft_plantillas_historico
...
log("Inicio DDV ft_plantillas_historico", "INFO", "ft_plantillas_historico")
...
# ----------------------------------------------------------
try:
    dbutils.widgets.text("prm_pipelineid",   "30")
    ...
    prm_ruta_hm_plantillas = dict_predecesores["hm_plantillas_equipo"]["Ruta_Predecesor"]
    ...
    log(f"Error parámetros: {e}", "ERROR", "ft_plantillas_historico")
...
try:
    ...
    prm_cols_hm_plantillas = cfg["cols_hm_plantillas"]
    prm_cols_md_plantillas = cfg["cols_md_plantillas"]
...
# ----------------------------------------------------------
try:
    df_hm_plantillas = read_delta_adls(spark, prm_ruta_hm_plantillas)
    df_md_plantillas = read_delta_adls(spark, prm_ruta_md_plantillas)
    df_dm_equipos    = read_delta_adls(spark, prm_ruta_dm_equipos)

    if prm_periodo > 0:
        df_hm_plantillas = df_hm_plantillas.filter(col("periodo") == prm_periodo)

    if is_dataframe_empty(df_hm_plantillas):
        raise Exception(f"hm_plantillas_equipo vacía: {prm_ruta_hm_plantillas}")
...
# ----------------------------------------------------------
cnt_entrada = df_hm_plantillas.count()
cnt_salida  = 0
cnt_nulos   = 0
try:
    df_final = carga_final_ft_plantillas_historico(
        df_hm_plantillas, prm_cols_hm_plantillas,
        df_md_plantillas, prm_cols_md_plantillas,
    ...
    log("ft_plantillas_historico cargado correctamente", "SUCCESS", entity_name)
except Exception as e:
...
log("Fin DDV ft_plantillas_historico", "INFO", entity_name)
