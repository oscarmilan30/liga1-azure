...
def carga_final_hm_partidos(
    ...
    # ================================================================
    # 2) RDV partidos → selección + dedup
    # ================================================================
    df_raw_sel = df_raw_partidos.select(
    ...
    df_hm = df_base.select(
        # id_partido determinístico: temporada + url completa (fragment #matchID = clave única FotMob por partido)
        sha2(
            concat_ws("-", col("temporada").cast("string"), col("url")),
            256
        ).alias("id_partido"),
